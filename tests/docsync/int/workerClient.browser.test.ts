...
test("the full client uploads offline edits from a worker and notifies another device", async () => {
  await testWrapper(async ({ docId, reference, otherDevice }) => {
    ...
    reference.addChild("from full worker client");
    ...
    await reference.assertIDBDoc({ doc: ["from full worker client"], ops: [] });
    await otherDevice.assertMemoryDoc(["from full worker client"]);
  });
});
