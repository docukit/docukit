...
2. Add broadcast channel:
   ...
   - Option 2: notify to the other tabs that there have been changes to that document, so that if they have it open, they can update it from indexedDB. This option would not allow an “inMemoryProvider,” but if someone did not want to persist in indexedDB (server-first RTC app), they could clear indexedDB and use it only as the communication channel.
