...
## 5. Replication state machine (replaces push flags)

This state machine replaces the following booleans:

- `_pushInProgress`
- `_inLocalWaiting`

These flags encode **mutual exclusion** and **queued intent** and must not be removed,
only modeled correctly.

### Replication states

- idle  
  No in-flight replication request.

- pushing  
  One replication request is currently in flight.
  No second request may start.

- pushing-and-dirty  
  A replication request is in flight **and** new local operations arrived.
  A follow-up push is required.

### Semantics

- Guarantees at most one in-flight request
- Guarantees no local changes are lost
- Preserves exact behavior of previous flags
...
- idle → pushing
- pushing → pushing-and-dirty
- pushing-and-dirty → pushing-and-dirty

On push success:

- pushing → idle
- pushing-and-dirty → pushing (immediately start next push)

On push failure:

- pushing → idle (or retry policy)
- pushing-and-dirty → pushing
