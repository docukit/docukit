        ...
        callback({
          ...
          saveOperations: async ({ docId, operations }) => {
            ...
            if (operations.length === 0) {
              ...
              return Math.max(latest?.clock ?? 0, doc?.clock ?? 0);
            }
...
The public examples use a separate [SQLite provider](https://github.com/docukit/docukit/blob/main/docs/collab-server/sqlite-provider.ts) with seven-day cleanup because demo documents are intentionally ephemeral.
...
In the client config, `provider` is a **factory** that receives an `Identity` and returns the provider instance:
...
DocSync passes a user identity to the provider. If DocSync has a cached
server-verified `userId`, it can open local storage before the network
connection completes. After the server authenticates the socket, the
server-verified `userId` remains authoritative.
...
Each user gets an isolated IndexedDB database named `docsync:v${schema}:${userId}`.
The built-in provider stores plaintext local docs and operations. Its `listClocks`
method reads an index without loading document contents. Omit `docIds` to list
all stored documents, or provide IDs to filter the result. Missing documents are
left out. This method is optional for custom providers.

**A storage schema change starts with an empty local database.** Documents already
saved on the server can be downloaded again. Local-only documents and edits not
yet sent to the server are not recovered automatically. Previously cached
documents must be downloaded again before they are available offline.

Earlier databases are neither migrated nor deleted. Tabs using them may continue
to sync, but the new provider does not read their data. Reopening the same schema
preserves its local data; an ordinary release without a schema change does not
reset storage.

If your app needs local encryption, implement or wrap a `ClientProvider` that
encrypts before writes and decrypts after reads.
