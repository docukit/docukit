...
A node is the fundamental building block of a DocNode document. Each node has an id, a type, an optional state object, and a position in a tree (parent, siblings, and children).
...
```ts
import { Doc, defineNode, string, number, boolean } from "@docukit/docnode";

// Paragragh with Uppercase is the NodeDefinition (recommended convention)
const Paragraph = defineNode({
  type: "paragraph",
  state: { text: string(""), rating: number(0), visible: boolean(true) },
});
...
The root node is the only one that DocNode automatically inserts. Every document has exactly one root node, accessible via `doc.root`. You cannot insert a new root node. Attempting to do so will throw.

The root node's ID is considered the document ID, and its type is considered the document type. They are synonymous.

```ts
import { Doc } from "@docukit/docnode";

// 1. Default type and id
const doc = new Doc();
const root = doc.root;
console.log(root.id); // random ulid, e.g. '01kdfwt92h5jqa8a0tc7w3jfc8'
console.log(root.type); // 'root' (default value)

// 2. Custom type and id
const doc2 = new Doc({
  type: "richtext-editor",
  id: "01kdfwy0qdzrq2ysfcpf8jgw0m", // must pass nodeIdGenerator.validate() - see /docs/nodes/id
});
const root2 = doc2.root;
console.log(root2.id); // '01kdfwy0qdzrq2ysfcpf8jgw0m' -> the docId is considered the root node id
console.log(root2.type); // 'richtext-editor' -> The doc type is considered the root node type
```

<Callout type="info">
  The other nodes do not use ulids as their default IDs. They use a more compact
  base64 ID, composed of the number of milliseconds that have passed since the
  document was created, 3 random characters, and a lamport timestamp.
</Callout>
...
```ts
import { string, number, boolean, defineNode, Doc } from "@docukit/docnode";
...
```ts
import { defineNode, string, Doc, type Extension } from "@docukit/docnode";

const TextNode = defineNode({ type: "text", state: { text: string("") } });
...
const doc = new Doc({ extensions: [BaseExtension, ColoredExtension] });
...
This merging also applies to the type of your root node, which allows enriching the state of the root node, useful for adding metadata to the document.
