...
The `changeEvent` contains those 3 properties plus `origin`, while the `normalizeEvent` contains only the `diff`:
...
doc.onChange(({ operations, inverseOperations, diff, origin }) => {
  console.log("change", operations, inverseOperations, diff, origin);
});
```
...
required: true,
    ...
    origin: {
      ...
      description:
        "Optional transaction origin. A common use case is tagging remote changes when replaying operations with `doc.applyOperations(operations, origin)`. DocNode's UndoManager ignores changes whose origin starts with remote, so they are not added to the undo stack.",
required: false,
},
}}
/>
...
Passing an `origin` is useful when your app needs to distinguish remote changes
from local ones.
